...
df_revgeo_h = df_revgeo.hint(hint).join(df_income_highest3.hint(hint), ['Zip Code'], how='inner')
df_revgeo_h.explain()
df_revgeo_h = df_revgeo_h.select(['Zip Code', 'LAT', 'LON'])

# default: BroadcastHashJoin
df_revgeo_l = df_revgeo.hint(hint).join(df_income_lowest3.hint(hint), ['Zip Code'], how='inner')
df_revgeo_l.explain()
...
df_join_h = df.hint(hint).join(df_revgeo_h.hint(hint), ['LAT', 'LON'], how='inner')
df_join_h.explain()
...
df_join_h = df_join_h.hint(hint).join(df_desc_discr.hint(hint), 'Vict Descent', how='inner')
df_join_h.explain()
...
# default: SortMergeJoin ( BroadcastHashJoin )
df_join_l = df.hint(hint).join(df_revgeo_l.hint(hint), ['LAT', 'LON'], how='inner')
df_join_l.explain()
...
# default: SortMergeJoin( SortMergeJoin ( BroadcastHashJoin ) )
df_join_l = df_join_l.hint(hint).join(df_desc_discr.hint(hint), 'Vict Descent', how='inner')
df_join_l.explain()
