...
df_revgeo_h = df_revgeo.join(df_income_highest3.hint('BROADCAST'), ['Zip Code'], how='inner')
df_revgeo_h.explain()
df_revgeo_h = df_revgeo_h.select(['Zip Code', 'LAT', 'LON'])

# default: BroadcastHashJoin
df_revgeo_l = df_revgeo.join(df_income_lowest3.hint('BROADCAST'), ['Zip Code'], how='inner')
df_revgeo_l.explain()
...
df_join_h = df.join(df_revgeo_h.hint('SHUFFLE_HASH'), ['LAT', 'LON'], how='inner')
df_join_h.explain()
...
df_join_h = df_join_h.join(df_desc_discr.hint('BROADCAST'), 'Vict Descent', how='inner')
df_join_h.explain()
...
# default: SortMergeJoin ( BroadcastHashJoin )
df_join_l = df.join(df_revgeo_l.hint('SHUFFLE_HASH'), ['LAT', 'LON'], how='inner')
df_join_l.explain()
...
# default: SortMergeJoin( SortMergeJoin ( BroadcastHashJoin ) )
df_join_l = df_join_l.join(df_desc_discr.hint('BROADCAST'), 'Vict Descent', how='inner')
df_join_l.explain()
